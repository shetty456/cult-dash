...
export default function RevenueMini({ filters }: { filters: GlobalFilters }) {
  const [rows, setRows] = useState<DailyRow[]>([]);
  const filterKey = JSON.stringify(filters);

  useEffect(() => {
    const qs = new URLSearchParams(filters as Record<string, string>);
    fetch(`/api/revenue?${qs}`).then(r => r.json()).then(d => setRows(d.daily ?? []));
  ...
  return (
    <ResponsiveContainer width="100%" height={180}>
      <AreaChart data={rows} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
        <defs>
          <linearGradient id="revMiniGrad" x1="0" y1="0" x2="0" y2="1">
        ...
        <XAxis
          ...
          tickLine={false}
        />
        <YAxis tick={{ fill: '#4b5563', fontSize: 9 }} axisLine={false} tickLine={false} width={36} tickFormatter={fmtRev} />
        <Tooltip
          contentStyle={{ background: '#1e1e1e', border: '1px solid #3a3a3a', borderRadius: 6, fontSize: 11 }}
          labelFormatter={l => fmt(String(l))}
          formatter={(val, name) => [fmtRev(Number(val ?? 0)), name === 'revenue' ? 'Daily Rev' : 'Cum. MRR']}
        />
        <Area type="monotone" dataKey="revenue" stroke="#10b981" strokeWidth={1.5} fill="url(#revMiniGrad)" dot={false} isAnimationActive={false} />
        <Area type="monotone" dataKey="mrr" stroke="#a78bfa" strokeWidth={1.5} fill="url(#mrrMiniGrad)" dot={false} isAnimationActive={false} />
      </AreaChart>
    </ResponsiveContainer>
  );
}
